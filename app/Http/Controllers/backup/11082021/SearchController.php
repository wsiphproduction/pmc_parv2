...
{   
    ...
    public function fetch_item(Request $request)
    {
        ...
        {
            $query = $request->get('query');
            $data  = Items::where('inv_code','=','SAF')->where('description','LIKE', "$query%")->get();
            
            $output = '<ul class="dropdown-menu wd-100p" style="display:block; position:relative">';

            foreach ($data as $row) {
                $output .= "<li class='item_li_".$request->cNum."'><a href='#'>".$row->id." => ".$row->description."</a></li>";
            }
            $output .= '</ul>';
    ...
    public function items_to_par(Request $req)
    {
        ...
        if($type == 'transfer'){
            ...
            $items = Items::whereIn('id', function($query){ $query->select('item','id')->from('accountabilityDetails'); } )
                ->where('description','LIKE', "%$search%")
                ->get();

        } else {

            $items  = Items::where('description','LIKE',"%$search%")->orWhere('serial_no','LIKE',"$search%")->get();
